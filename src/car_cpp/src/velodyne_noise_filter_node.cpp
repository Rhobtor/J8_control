...
public:
  ...
  : Node("velodyne_noise_filter")
  {
    ...
    horizontal_support_enabled_ = this->declare_parameter<bool>("horizontal_support_enabled", false);
    horizontal_support_radius_voxels_ = this->declare_parameter<int>("horizontal_support_radius_voxels", 1);
    min_horizontal_support_points_ = this->declare_parameter<int>("min_horizontal_support_points", 2);
    ...
    if (horizontal_support_radius_voxels_ < 0) {
      RCLCPP_WARN(this->get_logger(), "horizontal_support_radius_voxels < 0. Se fuerza a 0");
      horizontal_support_radius_voxels_ = 0;
    }
    if (min_horizontal_support_points_ < 1) {
      RCLCPP_WARN(this->get_logger(), "min_horizontal_support_points < 1. Se fuerza a 1");
      min_horizontal_support_points_ = 1;
    }
    ...
    RCLCPP_INFO(
      ...
      "  horizontal_support: %s | radius: %d | min_points: %d\n"
      "  range: [%.2f, %.2f] | z: [%.2f, %.2f]\n"
      ...
      horizontal_support_enabled_ ? "ON" : "OFF",
      horizontal_support_radius_voxels_, min_horizontal_support_points_,
...
private:
  ...
  using HorizontalCountMap = std::unordered_map<std::array<int, 2>, int, Key2Hasher, Key2Equal>;
  ...
  int horizontalSupportCountForKey(
    const HorizontalCountMap & support_counts,
    const std::array<int, 2> & key) const
  {
    int support = 0;
    for (int dx = -horizontal_support_radius_voxels_;
      dx <= horizontal_support_radius_voxels_; ++dx) {
      for (int dy = -horizontal_support_radius_voxels_;
        dy <= horizontal_support_radius_voxels_; ++dy) {
        const std::array<int, 2> neighbor{{key[0] + dx, key[1] + dy}};
        const auto it = support_counts.find(neighbor);
        if (it != support_counts.end()) {
          support += it->second;
        }
  ...
  void cloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
  {
    ...
    HorizontalCountMap horizontal_support_counts;
    horizontal_support_counts.reserve(npts / 2 + 1);
    ...
    for (std::size_t i = 0; i < npts; ++i) {
      ...
      ++horizontal_support_counts[{{
        static_cast<int>(std::floor(x * inv_support_leaf)),
        static_cast<int>(std::floor(y * inv_support_leaf))}}];
    }
    ...
    std::size_t horizontal_support_rejected = 0;
    for (const auto & point : candidates) {
      ...
      if (horizontal_support_enabled_) {
        const std::array<int, 2> horizontal_key{{support_key[0], support_key[1]}};
        if (horizontalSupportCountForKey(horizontal_support_counts, horizontal_key) <
          min_horizontal_support_points_) {
          ++horizontal_support_rejected;
          continue;
        }
      }
    ...
    if (debug_log_) {
      RCLCPP_INFO_THROTTLE(
        this->get_logger(), *this->get_clock(), 2000,
        "in=%zu | candidates=%zu | support_cells=%zu | out_voxels=%zu | out=%u | rej[nan=%zu,origin=%zu,range=%zu,z=%zu,box=%zu,support=%zu,horizontal=%zu,density=%zu,final_box=%zu]",
        npts, candidates.size(), support_counts.size(), out_voxels.size(), out.width,
        nan_rejected, origin_rejected, range_rejected, z_rejected, box_rejected,
        support_rejected, horizontal_support_rejected, density_rejected, final_box_rejected);
    }
  }
  ...
  bool horizontal_support_enabled_;
  int horizontal_support_radius_voxels_;
  int min_horizontal_support_points_;
