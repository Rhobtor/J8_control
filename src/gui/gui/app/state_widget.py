# cuadriga_gui/app/state_widget.py
import math
import json
import os
import re
...
class HUDWidget(QWidget):
    ...
    def paintEvent(self, ev):
        ...
        # Fondo + horizonte (opcional). Para el cuadriga (tierra) lo desactivamos por defecto.
        p.setPen(Qt.NoPen)
...
class StateSignals(QObject):
    ...
    gps      = Signal(str)                   # JSON dict {'fix':int|str|None,'sats':int|None,'hdop':float|None}
    stats    = Signal(str)                   # JSON dict para StatsPanel.update_stats(**d)
    ...
    gps_pose = Signal(float, float, float)
    detected_persons = Signal(str)
    # FSM feedback (legacy GUI_pkg contract)
    fsm_mode = Signal(int)
    possible_transitions = Signal(str)  # JSON list[int]
...
class TelemetryNode(Node):
    ...
    def _on_imu(self, msg: Imu):
        ...
        # Si ya conocemos lat/lon, podemos publicar una georreferencia útil
        # para el mapa aunque la calidad del fix no sea la máxima.
    ...
    def _on_odom(self, msg: Odometry):
        ...
        self._emit_json(self._sig.stats, payload)
    ...
    def _on_fix(self, msg: NavSatFix):
        ...
        self._emit_json(self._sig.gps, {'fix': int(msg.status.status), 'sats': self._cached_sats, 'hdop': self._cached_hdop})

        # Publica la pose en cuanto tengamos lat/lon. Si ya existe un heading
        # reciente de IMU/odometría, lo reutilizamos para poder georreferenciar
        # detecciones locales sin esperar al siguiente ciclo de orientación.
        if self._last_bearing_deg is None:
            self._sig.gps_pose.emit(float(self._lat), float(self._lon), math.nan)
        else:
            self._sig.gps_pose.emit(float(self._lat), float(self._lon), float(self._last_bearing_deg))

        if self._guided_lat is not None and self._guided_lon is not None:
            dist_wp = haversine_m(self._lat, self._lon, self._guided_lat, self._guided_lon)
            self._emit_json(self._sig.stats, {'dist_wp': dist_wp, 'dist_mav': dist_wp})
    ...
    def _on_hdop(self, msg):
        try:
            ...
            self._emit_json(self._sig.gps, {'fix': None, 'sats': self._cached_sats, 'hdop': self._cached_hdop})
        except Exception:
            pass
    ...
    def _on_gps_vel(self, msg: Vector3Stamped):
        try:
            ...
            self._emit_json(self._sig.stats, {'gps_speed': self._cached_gps_speed})
        except Exception:
            pass

    def _on_cte(self, msg):
        try:
            self._emit_json(self._sig.stats, {'cte': float(msg.data)})
        except Exception:
            pass

    def _on_lookahead(self, msg):
        try:
            self._emit_json(self._sig.stats, {'lookahead': float(msg.data)})
        except Exception:
            pass
    ...
    def _on_min_dist_path(self, msg):
        try:
            ...
            self._emit_json(self._sig.stats, {'min_dist_path': float(msg.data)})
        except Exception:
            pass
    ...
    def _on_possible_transitions(self, msg):
        try:
            ...
            try:
                ...
                lst = []
            self._emit_json(self._sig.possible_transitions, lst)
        except Exception:
            pass
    ...
    def _on_detected_persons_latlon(self, msg: PoseArray):
        ...
        try:
            ...
            return
        self._emit_json(self._sig.detected_persons, points)
...
# poblamos el combo desde /cuadriga/possible_transitions.
...
class MissionWidget(QWidget):
    ...
    def __init__(self, ros=None, js_call: Optional[Callable[[str], None]] = None):
        ...
        self._fsm_service_poll_timer = QTimer(self)
        self._fsm_service_poll_timer.setInterval(1000)
        self._fsm_service_poll_timer.timeout.connect(self._poll_fsm_feedback_services)
    ...
    def set_ros(self, ros_side):
        self._ros = ros_side
        if hasattr(self._ros, 'set_fsm_feedback_signals'):
            self._ros.set_fsm_feedback_signals(self.state._signals)
        if self._ros is None:
            self._fsm_service_poll_timer.stop()
        elif not self._fsm_service_poll_timer.isActive():
    ...
    def _on_robot_gps_pose(self, lat: float, lon: float, heading_deg):
        ...
        if heading_deg is not None and math.isfinite(float(heading_deg)):
            try:
    ...
    def _maybe_stop_fsm_service_poll(self):
        if self._fsm_mode is None:
            return
        if not self._possible_transitions:
            return
        if self._fsm_service_poll_timer.isActive():
            self._fsm_service_poll_timer.stop()
    ...
    def _export_goal_points_to_circuit_yaml(self, planned_latlon: List[Tuple[float, float]]):
        ...
        print(f"[cuadriga_gui] Wrote {len(goal_pts)} goal_points to {yaml_path}")
